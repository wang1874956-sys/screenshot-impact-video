...
带两块大组件的手机桌面也可以做短节奏试片：完整保留组件中的照片与文字，交替切换焦点，让图标承担更快的穿行动作。该方向已在一张 8 秒试片上得到用户认可；不代表所有图片都应套用相同的时长和轨迹。公开仓库只包含虚构演示素材，不包含这张私人桌面截图。
...
想先复现公开演示，可把上面的 source 改成 `demo-source.png` 并选择 `cards`；这张演示原图与卡片模板坐标匹配，可以直接构建。仓库示例页为专门制作的虚构页面。
